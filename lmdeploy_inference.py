...
def evaluate(cfg: DictConfig, checkpoint: str, prefix="", _split="dev"):
    ...
    for i in trange(len(dataset)):
        ...
        if "image" in inputs:
            all_prompts.append((text, inputs.pop("image")))
        else:
            all_prompts.append(text)
        all_meta_data.append(inputs.pop("meta_data"))

    # sampling_params: SamplingParams = hydra.utils.instantiate(cfg.sampling_params)
    gen_config: GenerationConfig = hydra.utils.instantiate(cfg.generation_config)
    # logger.warning(f"Sampling params: {sampling_params}")
    logger.warning(f"Generation config: {gen_config}")

    backend_config: Union[PytorchEngineConfig, TurbomindEngineConfig] = hydra.utils.instantiate(cfg.backend_config)

    pipe = pipeline(checkpoint,
                    cfg.model_name,
                    backend_config=backend_config)

    outputs = pipe(all_prompts, gen_config=gen_config)
...
def main(cfg: DictConfig):
    ...
    for checkpoint in checkpoints:
        ...
        result = evaluate(cfg, checkpoint, prefix=prefix, _split=split)
