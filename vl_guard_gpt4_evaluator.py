...
def main():
    ...
    parser.add_argument("--engine", "-e", choices=["gpt-4o", "gpt-4o-mini"],
    ...
    with open(args.file_path, 'r') as f:
        inputs = json.load(f)
    inputs = [item for item in inputs if item["category"] == "safe-image-safe-instruction"]
    print(len(inputs))
    inputs = {"/".join(item["image"].split("/")[-2:]): item["response"] for item in inputs}
